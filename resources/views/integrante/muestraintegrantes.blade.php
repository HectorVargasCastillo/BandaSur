        ...
        <h3 class="box-title">Banda :  {{$datos_banda->nombre}}  [{{$datos_banda->id}}]  </h3>

        <div>
            <button type="button" onclick="location.href = '{{ url('/integrantes/creaintegranteBanda/'.$datos_banda->id) }}'" class="btn btn-success">Agregar Integrante</button>
        </div>
        <div><h1></h1></div>
        ...
        <table border="2" sTYLE="table-layout:fixed">
            ...
            @foreach($muestra_integrante as $integrante)
        ...
        <div>
            <button type="button" onclick="location.href = '{{ url('/bandas') }}'" class="btn btn-info">Salir</button>
        </div>
